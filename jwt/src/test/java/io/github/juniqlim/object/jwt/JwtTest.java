package io.github.juniqlim.object.jwt;

import org.junit.jupiter.api.Test;

import java.security.KeyFactory;
import java.security.NoSuchAlgorithmException;
import java.security.PrivateKey;
import java.security.spec.InvalidKeySpecException;
import java.security.spec.PKCS8EncodedKeySpec;
import java.util.Base64;
import java.util.HashMap;

import static org.junit.jupiter.api.Assertions.assertTrue;

class JwtTest {
    @Test
    void test() throws NoSuchAlgorithmException, InvalidKeySpecException {
        String StringPrivateKey = "";
        PrivateKey privateKey = KeyFactory.getInstance("RSA")
                .generatePrivate(new PKCS8EncodedKeySpec(Base64.getDecoder().decode(StringPrivateKey)));

        String token = new Jwt.Jws(privateKey).token();
        System.out.println(token);
        assertTrue(token.length() > 0);
    }

    @Test
    void withRequest() throws NoSuchAlgorithmException, InvalidKeySpecException {
        String StringPrivateKey = "";
        PrivateKey privateKey = KeyFactory.getInstance("RSA")
            .generatePrivate(new PKCS8EncodedKeySpec(Base64.getDecoder().decode(StringPrivateKey)));

        HashMap<String, Object> payload = new HashMap<>();
        payload.put("id", "1234567890");
        String token = new Jwt.Jws(privateKey).token(new Jwt.Jws.Request(60, payload));
        System.out.println(token);
        assertTrue(token.length() > 0);
    }

    @Test
    void withClaimsMap() throws NoSuchAlgorithmException, InvalidKeySpecException {
        String StringPrivateKey = "";
        PrivateKey privateKey = KeyFactory.getInstance("RSA")
            .generatePrivate(new PKCS8EncodedKeySpec(Base64.getDecoder().decode(StringPrivateKey)));

        String token = new Jwt.Jws(privateKey).token(
            new HashMap<String, Object>() {{
                put("id", "1234567890");
            }}
        );
        System.out.println(token);
        assertTrue(token.length() > 0);
    }
}
